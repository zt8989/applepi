...
- **Slash commands (core capability)** — registered via `Harness.registerSlashCommand(name, handler)`; core 自注册 `/level`，内置 `/config`, `/resume <id>`, `/new`, `/sessions` (list `~/.applepi/sessions/<workspace>/`), `/help`, `/exit`. Web drives the same core methods.（CLI REPL 已删。）
