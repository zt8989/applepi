...
- **Bundle（能力包）** — a self-contained capability unit. `base` = exactly `bash` + `str_replace_editor` (two tools, minimal prompt; no memory/skills/plan/goal/subagent). `standard` = a self-contained full set (its own bash/sre reusing shared tool impls, plus memory/skills/web/plan/goal/subagent/workflow/todo/ask_user; its own prompt). **Siblings — `standard` does not inherit `base`.** There is **no `extends`** concept.
...
- **App（应用）** — `web`, `tui` (design-only). An app is an **interface**, not a bundle/mode: it hosts a mode selection and overlays its own interface capabilities (web tools/env declared in `apps/web`); `tui` is modeled but not implemented. Interface axis (web/tui) × capability axis (base/standard) are orthogonal.
...
- **Message line** — `kind:"message"` line mirroring an LLM message (`role`: system|user|assistant|tool). The first system message is the flat system prompt (persisted at session start / `/reload` by the app; each turn uses a freshly-assembled prompt, ADR-0015).
...
- **Slash commands (core capability, not CLI-only)** — registered via `Harness.registerSlashCommand(name, handler)`; core 自注册 `/level`，内置 `/reload`, `/resume <id>`, `/new`, `/sessions` (list `~/.applepi/sessions/<workspace>/`), `/help`, `/exit`. A future web UI drives the same core methods.
- **REPL** — the CLI REPL reads one user turn per line (Enter submits); multi-line via `/paste` or shell heredoc. Ctrl-D / `/exit` quits.
- **Reload（重载）** — `/reload`：**app 层**插件重载（ADR-0015，不再是 new Harness）——撤销插件层已注册工具 → 重扫 `extensions/` → re-register → 重建扁平提示词并按 `reload/start`+新 system 消息+`reload/end` 持久化（replay 语义：最新 system 替换 message[0]）。bundle/mode、`session.scratch` + `session.history` 不变。
...
- `apps/agent` (`@applepi/agent`) — the local agent: `main.ts` 选 bundle（`--mode base|standard`，默认 standard）+ 工具注册 + 扁平提示词组装 + 插件加载器（`plugins.ts` 扫描 `extensions/`），运行 REPL；`scripts/` 六个 key-free 检查。
- `apps/web` (`@applepi/web`) — Next.js 界面：每 (workspace, mode) 缓存一个 Harness，装配 bundle + capabilities、扁平提示词组装、mode 建会话时选定。

Dependency graph (one direction): `@applepi/agent → @applepi/bundle → @applepi/core` 与 `@applepi/bundle → @applepi/extensions → @applepi/core`；`@applepi/web` 依赖 core + bundle。Cross-package imports use **package names** resolved to `dist/` via each package's `exports` (Q3=a); dev/test run **build-first** (Q4=a); each package has its own `tsconfig.json` extending a shared base (Q5=a); the root `package.json` orchestrates `build`/`dev`/`test`/`verify` (Q6=a).
...
- **无特权打包** — permission **声明段**进 bundle（base/standard 各自声明贴合自身工具集）；security **强制机制**（级别模型/ctx 注入/工具自决）仍留 core、作为工具执行缝上的 adapter，不写提示词文案。
- **Bundle/Mode/App** — `base`（仅 bash+sre 两工具 + 极简提示词）= `standard`（自包含全集，**不继承 base**）是兄弟 bundle；`mode` = 被 app 托管的 bundle（base/standard 既是 bundle 也是 mode）；`web`/`tui` 是 **app**（接口，不是 bundle/mode），tui 只设计不实现；接口轴 × 能力轴正交。**无 `extends`**。
...
- **模式选择** — 仅在新建会话时选（web 新对话下拉 / CLI 参数），非热切换、非 `mode/set` 事件；记入 `session.config.mode`（构建期、会话内不可变），恢复时按它重建 spec。
