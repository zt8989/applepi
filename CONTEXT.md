...
- **Reference tool** — a concrete tool shipped by the `@applepi/extension` package as a replaceable reference implementation (not core): `bash` and `str_replace_editor`.
...
- **Event（事件）** — `kind:"event"` line recording a lifecycle record in the merged `event` field. **ADR-0018（2026-08-22 实现）后 jsonl 只剩 LM 过程四族**：可暂停过程成对 `turn/start|end` / `tool_call/start|end` / `tool_result/start|end`，原子单点 `system_prompt/set`（分类原则：可暂停过程必须有 start/end 区间，中断 = 开放区间；不允许暂停的原子操作只留单事件点）。UI 状态事件（`title/set` / `pin/set` / `notify/set`）与一次性审计事件（`tool/approval-pending`）已删除——前者迁旁挂 `<id>.meta.json>`（last-wins），后者的语义由未闭合 `tool_call` 区间替代（存储原语 `pendingToolCall()`，`lastEvent` 退役）；旧会话文件里的 UI 事件不再读取（无向后兼容读取器，无迁移脚本）。历史：`type`+`phase` 合并进 `event`（ADR-0006）；`system_prompt/*` 旧族与 `skill/start|end` 随 ADR-0015 移除；`mcp` 随 mcp 特性删除（Q11）；配置类状态随 ADR-0016 迁 `<id>.config.json>`（实现完成 2026-08-21）。**ADR-0015 起无 `emit` 事件总线**：事件由 app / 工具直接 `SessionStore.appendEvent` 写入 jsonl——`appendEvent` 就是存储原语，不存在 core 内置事件处理器。
...
- **思考过程块（Thinking block）** — assistant 消息里 `{type:'reasoning', text}` 部分，前端渲染为**默认折叠**的可展开「思考过程」块（位于回复正文上方），随消息持久化（core 在 assistant 消息中追加 reasoning part）。流式开启：`loop.ts` 的 `mergeIntoDataStream` `sendReasoning: true`，客户端 `onReasoningPart` 攒 delta。
...
- **包名约定（packaging convention, 2026-08-22）** — 包名 = 该包导出的**核心概念，单数**：`core` / `bundle` / `extension` / `server` / `tui` / `web`。`@applepi/extensions` 于 2026-08-22 更名为 `@applepi/extension`（目录 `packages/extension`），消除唯一复数异常、与领域词「Extension（扩展）」一致；ADR-0005/0015 等正文沿用旧名，见各 ADR 修订注记。
...
- `packages/extension` (`@applepi/extension`) — reference tools (bash / str_replace_editor, each self-determining per level) + capability factories `createMemory` / `createSkills`（`getCapability` 注册表）。Security mechanism lives in **core** since ADR-0009 (was the security extension here, ADR-0007).
- `apps/web` (`@applepi/web`) — **唯一界面**（CLI 已删）：Next.js，每 (workspace, mode) 缓存一个 Harness，装配 bundle + capabilities、扁平提示词组装、mode 建会话时选定。

Dependency graph (one direction): `@applepi/web → @applepi/bundle → @applepi/core` 与 `@applepi/bundle → @applepi/extension → @applepi/core`。Cross-package imports use **package names** resolved to `dist/` via each package's `exports` (Q3=a); dev/test run **build-first** (Q4=a); each package has its own `tsconfig.json` extending a shared base (Q5=a); the root `package.json` orchestrates `build`/`dev`/`test`/`verify` (Q6=a).
...
- **安全机制归属（Q3=b + 补充）** — **permission 由 core 内置**：core 强制保证 tool 栈最外层审计位存在，权限级别模型 / permissionMiddleware / ToolFilter / 权限声明段落 / `/level` 内聚进 core，不再是 `@applepi/extension` 的可选扩展。部分逆转 ADR-0005（"安全移出 core"），本质是「注册约定 → core 机制」的演进，需 ADR-0009 记录。
...
- **工具批准（tool approval）** — web 会话对工具执行采用**前端批准**：`ToolSpec.approval`（`auto`/`ask`/按参数函数，缺省 `ask`）分类；`ask` 工具暂停——其 `tool_call` 开放区间即待审批状态（ADR-0018，无专门事件）；`POST /api/chat/approve` 扫描未闭合区间校验后从暂停点续跑，决策写 `tool_call/end { decision }` 关闭区间（**不重跑 LLM**，jsonl 即 loop 状态）。读类自动执行（memory_read / skill_load / view / bash 只读命令），写/执行类须批准；**拒绝 = 工具结果回填模型**（模型可自愈）。
...
- **侧栏树**：品牌「applepi π」→ 新对话 → 「空间 (N)」汇总头 → 按工作区分组的会话树（folder + ⌄ 折叠，默认展开，每工作区 5 条 + 查看更多；活跃会话浅色高亮，hover 显示 bell/archive/⋯ 动作：重命名 / 置顶 / 导出 jsonl / 通知标志）。会话元数据存旁挂 `<id>.meta.json`（ADR-0018，last-wins：title 缺省取首条 user 消息截断 40 字；pinned / notify 缺省 false；jsonl 不再有 UI 事件）；归档 = 移入 `.archive/` 子目录。
...
- **打包** — 新建 `packages/bundle`（`base`/`standard` 纯声明 `(env)=>{prompt,tools}`）；`packages/extension` 保留参考工具实现；`packages/core` 变为模块集；`apps/*` 为应用。
