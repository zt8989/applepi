...
- **行结构（ADR-0006 精简后）**：
  - 事件行：`{"kind":"event","event":"title/set","payload":{...},"ts":<ISO>}`
    ...
    `pin/set`、`notify/set`、`tool/approval-pending` 等为原子事件）。配置类状态
    （level/reasoning/mode）不在 jsonl，在旁挂 `<id>.config.json>`（ADR-0016）。
...
- **共享消息契约（deepen #03）**：`packages/core/message.ts` 定义跨 core→web 的消息形状
  ...
  （`isErrorResult` 导出）。
  事件（`title/set`、`pin/set`、`notify/set`、
  `tool/approval-pending`、`reload/start|end` 等）由 app / 工具直接
  `store.appendEvent` 写入 jsonl——`appendEvent` 就是存储原语，无 core 内置事件处理器。
  > **事件语义演进（2026-08-22 设计，ADR-0018）**：jsonl 事件将按「可暂停过程 → start/end 对、
  > 不可暂停原子 → 单事件」重组为 `turn` / `tool_call` / `tool_result` / `system_prompt`
  四族；UI 元数据迁 `<id>.meta.json>`；挂起审批 = 未闭合区间推导。完整模型见下节
  > 「事件模型」，spec `.scratch/lifecycle-events/spec.md`；**未实现**，实现后本节正文同步更新。

## 事件模型（lifecycle events）

> 记录 2026-08-22 设计（ADR-0018，spec `.scratch/lifecycle-events/spec.md`）。**设计态，未实现**——下面「当前实现」正文（`title/set`、`pin/set`、`notify/set`、`tool/approval-pending` 事件与 `lastEvent` 原语）仍为现状；实现后本节以上正文同步改写。
...
| `turn/start` / `turn/end` | `end: { finishReason: 'stop' \| 'tool-calls' \| 'max-turns' \| 'error' }` | 一次 LLM 生成迭代（loop 一次 while 迭代）即一个 turn；`end` 不悬跨 HTTP 段；`tool-calls` = 暂停在待审批工具调用上（暂停本身由 tool_call 开放区间表达） |
...
- **挂起审批 = 未闭合区间**：按 `toolCallId` 配对 start/end，最早「有 start 无 end」的 `tool_call` 区间即当前待审批调用；`tool/approval-pending` 事件（含决策追写、同轮下一待审批追写）整体删除，同轮所有 ask 工具的开放区间天然构成「待审批集合」。已知歧义（接受）：执行中崩溃也表现为开放区间，会被误读为待审批。
- **UI 元数据旁挂**：`title/set` / `pin/set` / `notify/set` 迁入 `<session_id>.meta.json`（last-wins：`{ title?, pinned?, notify? }`，缺失视为无覆盖）；与 `<id>.config.json>`（ADR-0016 override-diff 语义）分文件。会话展示元数据原语输出语义不变，读取源切换。
