...
- **Schema (Q2=a)** — `{ "provider": "openai", "model": "gpt-4o-mini", "apiKey": "OPENAI_API_KEY" }`. Missing file → defaults (provider `openai`, model `gpt-4o-mini`; anthropic defaults `claude-3-5-sonnet-latest`).
...
- **Fail fast (Q6=a)** — when the resolved apiKey is unusable (no file, no secret, placeholder-only), startup (and `/config`) errors immediately with a message pointing at the two files, mirroring today's "OPENAI_API_KEY not set".
