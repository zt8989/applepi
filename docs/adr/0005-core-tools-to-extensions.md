...
- **`baseExtension` (Q6)** — `packages/extensions/base.ts` exports a single `SetupFn` that registers `bashTool` + `strReplaceEditorTool` and mounts `denylistMiddleware` at priority 1000, outermost. One line (`harness.registerExtension(baseExtension)`) reproduces the old `main.ts` wiring.
